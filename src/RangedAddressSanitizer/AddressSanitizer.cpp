...
struct AddressSanitizer : public FunctionPass {
  ...
  void instrumentAddress(Instruction *OrigIns, Instruction *InsertBefore,
                         ...
                         Value *SizeArgument);
  Value *verifyAddress(Instruction *InsertBefore, Value *Addr, uint32_t TypeSize);
...
INITIALIZE_PASS_BEGIN(AddressSanitizer, "asan",
    ...
    false, false)
INITIALIZE_PASS_DEPENDENCY(RangedAddressSanitizer)
...
Value* AddressSanitizer::verifyAddress(Instruction *InsertBefore,
                                       Value *Addr, uint32_t TypeSize)
{
  IRBuilder<> IRB(InsertBefore);
  Value *AddrLong = IRB.CreatePointerCast(Addr, IntptrTy);
  
  BasicBlock * entryBlock = InsertBefore->getParent();
  ...
  if (ClAlwaysSlowPath || (TypeSize < 8 * Granularity)) {
    LLVMContext & context = Cmp->getContext();
    ...
    IRB.SetInsertPoint(NextBB->getFirstNonPHI());
    
    PHINode * phi = IRB.CreatePHI(Cmp2->getType(), 2, "joinFastSlow");
    phi->addIncoming(Cmp2, CheckTerm->getParent()); // "then"
    phi->addIncoming(ConstantInt::getFalse(context), entryBlock); // "if" // == [Cmp]
    return phi;
  } else {
    return Cmp;
  } 
...
void AddressSanitizer::getAnalysisUsage(AnalysisUsage & AU) const
{
//     AU.addRequired<SelectivePageMigration>();
}
...
bool AddressSanitizer::runOnFunction(Function &F) {
  ...
  // infer safe value set from FASan (maybe should do this through metadata)
  RangedAddressSanitizer * fasanPass = getAnalysisIfAvailable<RangedAddressSanitizer>();
  if (!fasanPass) {
      return false;
  }
  ValueSet safeUsers;
  ValueSet forcedChecks;
  
  if (fasanPass && !getenv("FASAN_DISABLE")) {
      safeUsers = fasanPass->getSafeUses();
      forcedChecks = fasanPass->getForcedChecks();
      dbgs() << "FASan: enabled!\n";
  } else {
      dbgs() << "FASan: disabled!\n";
  }
  ...
  SmallVector<Instruction*, 16> ToVerify; // __fasan_verify
  SmallVector<Instruction*, 16> ToTouch; // __fasan_touch
  SmallVector<Instruction*, 8> NoReturnCalls;
  int NumAllocas = 0;
  bool IsWrite;

  // instrument fake uses
  for (Function::iterator FI = F.begin(), FE = F.end();
    ...
    for (BasicBlock::iterator BI = FI->begin(), BE = FI->end();
        ...
        if (CallInst * call = dyn_cast<CallInst>(it)) {
            if (call->getCalledFunction()->getName() == "__fasan_touch") {
                ToTouch.push_back(call);
            } else if (call->getCalledFunction()->getName() == "__fasan_verify") {
                ToVerify.push_back(call);
            }
        }
    }
  }
...
#if 1
      if (forcedChecks.find(BI) != forcedChecks.end()) {
          // keep forced checks
      }
      if (safeUsers.find(BI) != safeUsers.end()) continue;
#endif
  ...
  // Supplement __fasan_verify
  for (size_t i = 0, n = ToVerify.size(); i != n; i++) {
      Instruction * Inst = ToVerify[i];
      CallInst * verifyCall = dyn_cast<CallInst>(Inst);
      Value * addrVal = verifyCall->getOperand(0);      
      Value * safeVal = verifyAddress(verifyCall, addrVal, 8); // TODO fix type size
      verifyCall->replaceAllUsesWith(safeVal);
      verifyCall->eraseFromParent();
  }
  
  // Supplement __fasan_touch
  for (size_t i = 0, n = ToTouch.size(); i != n; i++) {
      Instruction * Inst = ToTouch[i];
      CallInst * touchCall = dyn_cast<CallInst>(Inst);
      Value * addrVal = touchCall->getOperand(0);
      Instruction * addrInst = cast<Instruction>(addrVal);
      
      instrumentAddress(addrInst, touchCall, addrVal, 8, false, 0); // TODO fix width      
      touchCall->eraseFromParent();
    }
  ...
  for (size_t i = 0, n = ToInstrument.size(); i != n; i++) {
    ...
    if (ClDebugMin < 0 || ClDebugMax < 0 ||
      ...
      if (isInterestingMemoryAccess(Inst, &IsWrite))
        ...
        BasicBlock * gepParent = gep->getParent();
        BasicBlock::iterator it = gepParent->begin();
        for (;&*it != gep; ++it) {}
        it++;
